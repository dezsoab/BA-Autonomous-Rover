...
from src.constants import (
    BAUD_RATE, HEADER_BYTE_1, HEADER_BYTE_2, PACKET_SIZE, PAYLOAD_SIZE,
    POINTS_PER_PACKET, ANGLE_DIVISOR, MM_TO_CM, LIDAR_OFFSET_DEG,
    SCAN_DURATION_SEC, MAX_VALID_DIST_CM, STOPPING_DIST_CM,
    SECTOR_BOUNDARIES, DEFAULT_PORT
)
...
class LidarStrategy(ObstacleStrategy):
    ...
    def _get_sector(self, angle):
        ...
        if angle >= SECTOR_BOUNDARIES['FRONT_START'] or angle < SECTOR_BOUNDARIES['FRONT_END']:
            return 'FRONT'
    ...
    def check_path(self):
        ...
        while (time.time() - start_time) < SCAN_DURATION_SEC: 
            if self.ser.in_waiting > PACKET_SIZE:
                if self.ser.read() == HEADER_BYTE_1 and self.ser.read() == HEADER_BYTE_2:
                    data = self.ser.read(PAYLOAD_SIZE)
                    if len(data) != PAYLOAD_SIZE: continue
                    ...
                    for i in range(POINTS_PER_PACKET):
                        raw_dist_pos = 4 + (i * 3)
                        dist_mm = struct.unpack('<H', data[raw_dist_pos : raw_dist_pos+2])[0]
                        ...
                        if 0 < dist_cm < MAX_VALID_DIST_CM: 
                            if sector == 'FRONT' and dist_cm < self.front_dist:
                                self.front_dist = dist_cm
                            elif sector == 'LEFT' and dist_cm < self.left_dist:
                                self.left_dist = dist_cm
                            elif sector == 'RIGHT' and dist_cm < self.right_dist:
