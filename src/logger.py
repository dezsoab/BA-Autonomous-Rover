...
class ThesisLogger:
    def __init__(self, mode):
        ...
        with open(self.filepath, mode='w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(["Timestamp", "Mode", "Front_Dist_cm", "Left_Dist_cm", "Right_Dist_cm", "Action", "Notes"])
    ...
    def log(self, mode, front, left, right, action, notes=""):
        try:
            with open(self.filepath, mode='a', newline='') as f:
                ...
                writer.writerow([timestamp, mode, f"{front:.1f}", f"{left:.1f}", f"{right:.1f}", action, notes])
